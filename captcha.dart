  ...
  void buatTitik() {
    stokWarna.forEach((key, value) {
      ...
      for (var i = 0; i < jumlah; i++) {
        ...
        daftarTitik.add({'x': x, 'y': y, 'ukuran': ukuranTitik, 'cat': catTitik, 'warna': key});
      }
    });
  }

  void onTapDown(TapDownDetails details) {
    var titikTerpilih = daftarTitik.firstWhere(
          (titik) =>
      details.localPosition.dx >= titik['x'] &&
          details.localPosition.dx <= titik['x'] + titik['ukuran'] &&
          details.localPosition.dy >= titik['y'] &&
          details.localPosition.dy <= titik['y'] + titik['ukuran'],
      orElse: () => <String, dynamic>{},
    );

    if (titikTerpilih.isNotEmpty) {
      if (titikTerpilih['warna'] == warnaYangDitanyakan) {
        setState(() {
          daftarTitik.remove(titikTerpilih);
          if (daftarTitik.every((titik) => titik['warna'] != warnaYangDitanyakan)) {
...
class CaptchaPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    daftarTitik.forEach((element) {
      canvas.drawCircle(Offset(element['x'], element['y']), element['ukuran'], element['cat']);
    });
  }
